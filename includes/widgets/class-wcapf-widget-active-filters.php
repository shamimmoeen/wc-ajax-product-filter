	...
	class WCAPF_Active_Filters_Widget extends WP_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				__('WC Ajax Active Filters', 'wc-ajax-product-filter'), // Name
				array('description' => __('Shows active filters so users can see and deactivate those.', 'wc-ajax-product-filter')) // Args
			);
		}
			...
			if (sizeof($active_filters) > 0) {
				...
				$html .= '<div class="wcapf-active-filters">';
					...
					foreach ($active_filters as $key => $active_filter) {
						...
						if ($key === 'keyword') {
							$html .= '<a href="javascript:void(0)" data-key="keyword">' . __('Search For: ', 'wc-ajax-product-filter') . $active_filter . '</a>';
						}

						if ($key === 'orderby') {
							$html .= '<a href="javascript:void(0)" data-key="orderby">' . __('Orderby: ', 'wc-ajax-product-filter') . $active_filter . '</a>';
						}

						if ($key === 'min_price') {
							$html .= '<a href="javascript:void(0)" data-key="min-price">' . __('Min Price: ', 'wc-ajax-product-filter') . $active_filter . '</a>';
						}

						if ($key === 'max_price') {
							$html .= '<a href="javascript:void(0)" data-key="max-price">' . __('Max Price: ', 'wc-ajax-product-filter') . $active_filter . '</a>';
						}
		...
		public function form($instance) {
			...
			<p>
				<label for="<?php echo $this->get_field_id('button_text'); ?>"><?php printf(__('Button Text:', 'wc-ajax-product-filter')); ?></label>

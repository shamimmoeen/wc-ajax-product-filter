	...
	class WCAPF_Price_Filter_Widget extends WP_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				__('WC Ajax Product Filter by Price', 'wc-ajax-product-filter'), // Name
				array('description' => __('Filter woocommerce products by price.', 'wc-ajax-product-filter')) // Args
			);
		}
			...
			if ($display_type === 'slider') {
				$html .= '<div class="wcapf-price-filter-wrapper">';
					...
					$html .= '<div class="slider-values">';
						$html .= '<p>' . __('Min Price', 'wc-ajax-product-filter') . ': <span class="wcapf-slider-value" id="wcapf-noui-slider-value-min"></span></p>';
						$html .= '<p>' . __('Max Price', 'wc-ajax-product-filter') . ': <span class="wcapf-slider-value" id="wcapf-noui-slider-value-max"></span></p>';
					$html .= '</div>';
				$html .= '</div>';
			}
		...
		public function form($instance) {
			...
			<p>
				<label for="<?php echo $this->get_field_id('display_type'); ?>"><?php printf(__('Display Type:', 'wc-ajax-product-filter')); ?></label>
				<select class="widefat price-filter-display-type" id="<?php echo $this->get_field_id('display_type'); ?>" name="<?php echo $this->get_field_name('display_type'); ?>">
					<option value="slider" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'slider') ? 'selected="selected"' : ''); ?>><?php printf(__('Slider', 'wc-ajax-product-filter')); ?></option>
			...
			<p class="price-list-currency <?php echo (!isset($instance['display_type']) || $instance['display_type'] === 'slider') ? 'hidden' : ''; ?>">
				...
				<label for="<?php echo $this->get_field_id('show_currency'); ?>"><?php printf(__('Show currency', 'wc-ajax-product-filter')); ?></label>
			</p>
			<div class="price-list-wrapper <?php echo (!isset($instance['display_type']) || $instance['display_type'] === 'slider') ? 'hidden' : ''; ?>">
				<?php if (isset($instance['price_list']) && !empty($instance['price_list'])): ?>
					<?php foreach ($instance['price_list'] as $price_list): ?>
						<p class="price-list">
							<input type="text" class="widefat min" name="<?php echo $this->get_field_name('price_list'); ?>[min][]" value="<?php echo $price_list['min']; ?>" placeholder="<?php printf(__('Min price', 'wc-ajax-product-filter')); ?>" />
							<input type="text" class="widefat to" name="<?php echo $this->get_field_name('price_list'); ?>[to][]" value="<?php echo $price_list['to']; ?>" placeholder="<?php printf(__('to', 'wc-ajax-product-filter')); ?>" />
							<input type="text" class="widefat max" name="<?php echo $this->get_field_name('price_list'); ?>[max][]" value="<?php echo $price_list['max']; ?>" placeholder="<?php printf(__('Max price', 'wc-ajax-product-filter')); ?>" />
							<a href="javascript:void(0)" class="remove-price-list">&times;</a>
						</p>
					<?php endforeach ?>
				<?php else: ?>
					<p class="price-list">
						<input type="text" class="widefat min" name="<?php echo $this->get_field_name('price_list'); ?>[min][]" value="" placeholder="<?php printf(__('Min price', 'wc-ajax-product-filter')); ?>" />
						<input type="text" class="widefat to" name="<?php echo $this->get_field_name('price_list'); ?>[to][]" value="" placeholder="<?php printf(__('to', 'wc-ajax-product-filter')); ?>" />
						<input type="text" class="widefat max" name="<?php echo $this->get_field_name('price_list'); ?>[max][]" value="" placeholder="<?php printf(__('Max price', 'wc-ajax-product-filter')); ?>" />
						<a href="javascript:void(0)" class="remove-price-list">&times;</a>
					</p>
				<?php endif ?>
			</div>
			<p class="add-price-list-button-wrapper <?php echo (!isset($instance['display_type']) || $instance['display_type'] === 'slider') ? 'hidden' : ''; ?>">
				<a href="javascript:void(0)" class="button add-price-list"><?php printf(__('Add', 'wc-ajax-product-filter')); ?></a>
			</p>
